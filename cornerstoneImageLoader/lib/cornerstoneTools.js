...
//modality = seriesModule.modality;
  ...
  for (var i = 0; i < toolData.data.length; i++) {
    ...
    if (data.invalidated === false) {
      ...
      //圆的面积 倍数400
      area = Math.PI * (ellipse.width * columnPixelSpacing / 2)/20 * (ellipse.height * rowPixelSpacing / 2)/20;
...
//}
  ...
  for (var i = 0; i < toolData.data.length; i++) {
    ...
    //长度修改 倍数20
    var _length = Math.sqrt(dx * dx + dy * dy)/20;
...
//}
  ...
  for (var i = 0; i < toolData.data.length; i++) {
    ...
    if (data.invalidated === false) {
      ...
      // Calculate the image area from the ellipse dimensions and pixel spacing
      //矩形面积 倍数400
      area = ellipse.width * columnPixelSpacing * (ellipse.height * rowPixelSpacing)/400;
...
exports.default = function (element, filename, mimetype) {
	...
	Showbo.Msg.wait('正在下发...','请等待');
  // Setting the default value for mimetype to image/png
  mimetype = mimetype || 'image/png';
  var canvas = _externalModules.external.$(element).find('canvas').get(0).toDataURL(mimetype);
  var conId=document.getElementById('conId').value;
  var patientNo=document.getElementById('patientNo').value;
  $.ajax({  
      type: "post",         
      url:$.ip+"/manCons/uploadImg",
      data: {"image":canvas,"conId":conId,"patientId":patientNo}, 
      dataType : 'json',
      async: true,  
      success: function (data) { 
    	  if(data.msg==1){
    		  Showbo.Msg.alert("下发成功！"); 
    	  }else if(data.msg==2){
    		  Showbo.Msg.alert("下发失败！"); 
    	  }else{
