...
Showbo.storageLeft = '50%';
Showbo.storageTop = '50%';
...
Showbo.Msg = {
	...
	createBtn: function(p, v, fn) {
		...
		btn.onclick = function() {
			Showbo.Msg.dvCT.innerHTML = '';
			Showbo.Msg.hide();
			Showbo.Msg.buttons && delete Showbo.Msg.buttons;
			if(fn) fn(p);
		}
		return btn;
	},
	alert: function(msg, fn, lightBox) {
		this.show({
			buttons: {
				yes: '确认'
			},
	...
	confirm: function(msg, fn, lightBox) {
	...
	show: function(cfg) {
		...
		this.dvMsgBox.style.minWidth = this.defaultWidth + 'px';
		...
		if(cfg.wait) {
			...
			if(!cfg.buttons || (!cfg.buttons.yes && !cfg.buttons.no)) {
				cfg.buttons = {
					yes: this.buttons.yes||'确定',
					no: this.buttons.no||'取消'
				};
			} else {
				cfg.buttons.yes = this.buttons&&this.buttons.yes?this.buttons.yes:cfg.buttons.yes;
				cfg.buttons.no = this.buttons&&this.buttons.no?this.buttons.no:cfg.buttons.no;
			}
			...
			if(cfg.buttons.yes) {
				this.dvBtns.appendChild(this.createBtn(true, cfg.buttons.yes, cfg.fn));
				if(cfg.buttons.no) this.dvBtns.appendChild(document.createTextNode('　'));
			}
			if(cfg.buttons.no) this.dvBtns.appendChild(this.createBtn(false, cfg.buttons.no, cfg.fn));
		}
		Showbo.initBodyScale();
		this.dvMsgBox.style.display = 'block';
		this.lightBox.style.display = 'block';
		this.lightBox.style.backgroundColor = '';
		cfg.lightBox = cfg.lightBox == true ? true : false;
		if(cfg.lightBox) this.lightBox.style.backgroundColor = 'transparent';
		this.onResize(false);
		Showbo.Msg.reposition();
	},
	hide: function() {
		Showbo.Msg.reposition();
	...
	onResize: function(isResize) {
		...
		Showbo.Msg.dvMsgBox.style.top = Showbo.dvMsgBoxTop;
		Showbo.Msg.dvMsgBox.style.left = Showbo.dvMsgBoxLeft;
		Showbo.Msg.dvMsgBox.style.transform = 'translate(-50%,-50%)';
	},
	reposition: function() {
		if (Showbo.dvMsgBoxTop!=Showbo.storageTop) {
			Showbo.storageTop = Showbo.dvMsgBoxTop;
			Showbo.Msg.dvMsgBox.style.top = Showbo.dvMsgBoxTop;
		}
		if (Showbo.dvMsgBoxLeft!=Showbo.storageLeft) {
			Showbo.storageLeft = Showbo.dvMsgBoxLeft;
			Showbo.Msg.dvMsgBox.style.left = Showbo.dvMsgBoxLeft;
		}
	}
}